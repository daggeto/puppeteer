...
class Page extends EventEmitter {
  ...
  async waitForResponse(urlOrPredicate, options = {}) {
    const timeout = typeof options.timeout === 'number' ? options.timeout : 30000;
    return helper.waitForEvent(this._networkManager, NetworkManager.Events.Response, async response => {
      if (helper.isString(urlOrPredicate))
        return (urlOrPredicate === response.url());
      if (typeof urlOrPredicate === 'function') {
        const predicateResult = urlOrPredicate(response);
        let result = false;

        if (predicateResult instanceof Promise) {
          result = await predicateResult;
        } else {
